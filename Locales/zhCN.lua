...
"|cffff0000注意|r: 物品的法术ID是\"法术ID\"而不是\"物品ID\"(你可以使用其他插件如\"idTip\"来获取), 并且, 你可以通过手动输入法术ID或者使用下面的下拉菜单选择光环来更新/删除(添加/删除按钮)已经存在的光环。另外, 持续时间和冷却时间的单位是秒。最后, 如果你想取消一个声音效果, 你可以选择\"None\"作为声音效果来取消。\n\n" ..
"|cffffff00输入|r: 添加光环时, 法术ID、持续时间和冷却时间是必填项。法术ID必须是正整数并且存在于游戏中, 持续时间和冷却时间必须是非负数(可以是小数/浮点数或零)。"
